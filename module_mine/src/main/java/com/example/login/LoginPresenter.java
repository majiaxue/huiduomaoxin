...
public class LoginPresenter extends BasePresenter<LoginView> {
    ...
    public void sendCode() {
        ...
        Observable<ResponseBody> observable = RetrofitUtil.getInstance().getApi(CommonResource.URL_9_4001).getData(CommonResource.WXLOGIN_CODE, map);
    ...
    public void login(String phone, String password) {
        if (!PhoneNumUtil.isMobileNO(phone)) {
            ...
            Observable observable = RetrofitUtil.getInstance().getApi(CommonResource.URL_9_4001).postData(CommonResource.LOGIN_PHONE, map);
            RetrofitUtil.getInstance().toSubscribe(observable, new OnMyCallBack(new OnDataListener() {
                @Override
                public void onSuccess(String result, String msg) {
                    UserInfoBean userInfoBean = new Gson().fromJson(result, new TypeToken<UserInfoBean>() {
