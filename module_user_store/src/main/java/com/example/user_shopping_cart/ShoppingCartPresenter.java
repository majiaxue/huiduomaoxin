...
//        Map map = MapUtil.getInstance().addParms("searchInfo", "两件套").build();
    ...
    public void setShoppingCartRec() {
        Observable<ResponseBody> cart = RetrofitUtil.getInstance().getApi(CommonResource.BASEURL_4001).getDataWithout(CommonResource.CARTLIST + "/" + SPUtil.getUserCode() + "/" + 1);
...
//                        getView().totalPrice(totalPrice);
    ...
    private void reviseStutas() {
        Observable<ResponseBody> observable = RetrofitUtil.getInstance().getApi(CommonResource.BASEURL_4001).postHeadWithList(CommonResource.REVISE_CART_ITEM, updateList, SPUtil.getToken());
        RetrofitUtil.getInstance().toSubscribe(observable, new OnMyCallBack(new OnDataListener() {
            @Override
            public void onSuccess(String result, String msg) {
...
//                getView().totalPrice(totalPrice);
    ...
    private void delete() {
        Observable<ResponseBody> observable = RetrofitUtil.getInstance().getApi(CommonResource.BASEURL_4001).postHeadWithout(CommonResource.DELETE_CART, SPUtil.getToken());
        RetrofitUtil.getInstance().toSubscribe(observable, new OnMyCallBack(new OnDataListener() {
            @Override
            public void onSuccess(String result, String msg) {
